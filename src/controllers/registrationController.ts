...
export class RegistrationController {
  ...
  async submitRegistration(req: Request, res: Response): Promise<void> {
    ...
    if (Object.keys(errors).length > 0) {
      res.status(400).render("registration", this.buildViewModel(formData, errors));
      return;
    }
    ...
    try {
      ...
      res.status(400).render("registration", this.buildViewModel(formData, { general: message }));
    }
  }
  ...
  private validate(formData: RegistrationFormData): RegistrationValidationErrors {
